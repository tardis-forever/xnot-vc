...
class XNOTNeighborsVC(nn.Module):
    ...
    def match(self, query_seq: Tensor, matching_set: Tensor, synth_set: Tensor = None,
              ...
              target_duration: float | None = None, device: str | None = None, algorithm: str = "knn", 
             max_steps: int = 1000, W: float = 1.0, x_not: XNot | None = None) -> Tensor:
        ...
        with k=`topk`. Inputs:
            ...
            - `algorithm`: either 'knn' or 'xnot' - how to find matching samples.
            - `max_steps`: not used for `algorithm` = 'knn', limit on the step count for `algorithm` = 'xnot'.
            - `W`: not used for `algorithm` = 'knn', constraint W >= 1 for `algorithm` = 'xnot'.
            - `x_not`: optional XNot checkpoint to use for `algorithm` = 'xnot'.
        Returns:
        ...
        if algorithm == "xnot":
            if x_not is None:
                ...
                x_not.fit(query_seq, matching_set, max_steps=max_steps, t_iters=10, cost="sq_cost", batch_size=64, W=W)
